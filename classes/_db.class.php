...
class db extends config
{
	...
	{
		$this->Connect($this->hostDB, $this->userDB, $this->passDB, $this->baseDB);
	...
	/*======================================================================*\
	Function:	FetchAll
	Descriiption: Возвращ массив, из всех строк подходящих под запрос
	\*======================================================================*/
	function FetchAll()
	{
		$num = 0;
		$dat = array();
		while ($data = mysqli_fetch_assoc($this->LastQuery)) {
			$dat[$num] = $data;
			$num++;
			$dat[$num] = $num + 1;
		}
